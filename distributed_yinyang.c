...
//Example compilation
//mpicc distributed_yinyang.c -lm -o distributed_yinyang
...
int main(int argc, char **argv) {
  ...
  for( int index=0; index<points_to_calc; index++ ) {
    ...
    // Have rank 0 copy its portion of the dataset into its local data
    if( my_rank==0 ) {
      for( int dim_index=0; dim_index<M; dim_index++ ) {
        local_data[index][dim_index] = dataset[index][dim_index];
      }
    }
  }

  // Have rank 0 distribute the dataset to the other rank's local_data
  if( my_rank==0 ) {
    int ranks_end;
    for( int index=1; index<nprocs; index++ ) {
      ranks_end = row_ends[ index ];
      for( int point_index=row_starts[ index ]; point_index<ranks_end; point_index++ ) {
        MPI_Isend( dataset[ point_index ], M, MPI_DOUBLE, index, 0, MPI_COMM_WORLD, &request );
      }
    }
    ...
    for( int point_index=0; point_index<points_to_calc; point_index++ ) {
      MPI_Recv( local_data[ point_index ], M, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status );
    }
  }
  ...
  for( int point_index=0; point_index<points_to_calc; point_index++ ) {
    ...
    min_distance = -1;
    for( int clust_index=0; clust_index<K; clust_index++ ) {
      distance = euclidianDistance( local_data[ point_index ], cluster_centers[ clust_index ], M );
    ...
    for( int dim_index=0; dim_index<M; dim_index++ ) {
      local_sums[ nearest_center ][ dim_index ] =
        local_sums[ nearest_center ][ dim_index ] + local_data[ point_index ][ dim_index ];
    }
  ...
  while( moved_center ) {
    ...
    for( int point_index=0; point_index<points_to_calc; point_index++ ) {
      ...
      if( global_lower_bound >= upper_bounds[ point_index ]
        ...
        for( int dim_index=0; dim_index<M; dim_index++ ) {
          local_sums[ clusters[ point_index ] ][ dim_index ] = local_sums[ clusters[ point_index ] ][ dim_index ] + local_data[ point_index+row_start ][ dim_index ];
        }
      ...
      // Failed the outer test, check all the group lower bounds
        ...
        local_counts[ nearest_center ] = local_counts[ nearest_center ] + 1;
        for( int dim_index=0; dim_index<M; dim_index++ ) {
          local_sums[ nearest_center ][ dim_index ] = local_sums[ nearest_center ][ dim_index ] + local_data[ point_index ][ dim_index ];
        }
