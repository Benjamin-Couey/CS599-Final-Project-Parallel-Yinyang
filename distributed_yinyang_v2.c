...
//Example compilation
//mpicc distributed_yinyang_v2.c -lm -o distributed_yinyang_v2
...
// Printing to console
//mpirun -np 2 -hostfile myhostfile.txt ./distributed_yinyang_v2 10 10 2 1 MSD_year_prediction_normalize_0_1_100k.txt 1

// Printing to file
//mpirun -np 2 -hostfile myhostfile.txt ./distributed_yinyang 10 100 90 1 MSD_year_prediction_normalize_0_1_100k.txt 2

// Testing with valgrind
// valgrind --track-origins=yes ./distributed_yinyang 2 10 90 1 MSD_year_prediction_normalize_0_1_100k.txt 0

//mpirun -np 2 -hostfile myhostfile.txt ./distributed_yinyang 10 10000 2 1 urbanGB.txt 2

//mpirun -np 2 -hostfile myhostfile.txt ./distributed_yinyang 10 360177 2 1 urbanGB.txt 2

//mpirun -np 2 -hostfile myhostfile.txt ./distributed_yinyang 10 10000 90 1 YearPredictionMSD.txt 2

//mpirun -np 2 -hostfile myhostfile.txt ./distributed_yinyang 10 515345 90 1 YearPredictionMSD.txt 2
...
int main(int argc, char **argv) {
  ...
  // Allocate the full dataset and import file into it
  // Only rank 0 will temporarily store the full dataset
  if( my_rank==0 ) {
    int dataset_length = N * M;

    dataset=(double*)malloc(sizeof(double)*dataset_length);
  ...
  // An array to store how many double each rank will be responsible for
  // This is the number of points the rank is responisble for times the dimensionality
  // of the data
  int * points_per_rank;
  int * doubles_per_rank;

  // An array to store the displacement of the data rank 0 will send out
  int * displacement;
  ...
  if( my_rank==0 ){
    ...
    points_per_rank = (int*)malloc(sizeof(int)*nprocs);
    doubles_per_rank = (int*)malloc(sizeof(int)*nprocs);
    displacement = (int*)malloc(sizeof(int)*nprocs);
    ...
    for( int index=0; index<nprocs; index++ ) {
      ...
      points_per_rank[ index ] = row_ends[ index ] - row_starts[ index ];
      doubles_per_rank[ index ] = points_per_rank[ index ] * M;

      // Calculate the displacement based on how many points the rank is responsible for
      displacement[ index ] = (index > 0) ? ( displacement[ index-1 ] + doubles_per_rank[ index-1 ] ) : 0;
    }
  ...
  points_to_calc = row_end - row_start;
  int local_data_length = points_to_calc * M;
  ...
  // Allocate local data which will store only the points each rank is responsible
  // for
  local_data=(double*)malloc(sizeof(double*)*local_data_length);

  // Have rank 0 distribute the data to each rank
  MPI_Scatterv( dataset, doubles_per_rank, displacement, MPI_DOUBLE, local_data, local_data_length, MPI_DOUBLE, 0, MPI_COMM_WORLD );
  ...
  // Using 5 iterations of the traditional Kmeans algorithm, sort the cluster centers
  // into T groups
  ...
  // Initialize the global_counts vector which stores how many points are in
  // each cluster
  ...
  // the first iteration
  // For each data point a rank is responsible for it will be checked against every cluster
  // center
  int nearest_center;
  ...
  for( int point_index=0; point_index<points_to_calc; point_index++ ) {
    ...
    for( int dim_index=0; dim_index<M; dim_index++ ) {
      local_sums[ nearest_center ][ dim_index ] =
        local_sums[ nearest_center ][ dim_index ] + local_data[ local_start + dim_index ];
    }
  ...
  while( moved_center ) {
    ...
    for( int point_index=0; point_index<points_to_calc; point_index++ ) {
      ...
      if( global_lower_bound >= upper_bounds[ point_index ]
        ...
        for( int dim_index=0; dim_index<M; dim_index++ ) {
          local_sums[ clusters[ point_index ] ][ dim_index ] = local_sums[ clusters[ point_index ] ][ dim_index ]
          + local_data[ local_start + dim_index ];
        }
      ...
      // Failed the outer test, check all the group lower bounds
        ...
        local_counts[ nearest_center ] = local_counts[ nearest_center ] + 1;
        for( int dim_index=0; dim_index<M; dim_index++ ) {
          local_sums[ nearest_center ][ dim_index ] = local_sums[ nearest_center ][ dim_index ] + local_data[ local_start + dim_index ];
        }
  ...
  if( my_rank == 0 ) {
    for ( int index=0; index<nprocs; index++) {
      displacement[ index ] = (index > 0) ? ( displacement[ index-1 ] + points_per_rank[ index-1 ] ) : 0;
    }
  }

  // Gather all the values in all the ranks' sendBuffers in the rank that owns
  // this bucket
  MPI_Gatherv( clusters, points_to_calc, MPI_INT, global_clusters, points_per_rank, displacement, MPI_INT, 0, MPI_COMM_WORLD );

  if( my_rank==0 ) {
    free( points_per_rank );
    free( doubles_per_rank );
    free( displacement );
  }
...
int importDataset(char * fname, int N, int M, double * dataset)
{
    ...
    while (fgets(buf, 4096, fp) && rowCnt<N) {
        ...
        while (field) {
          ...
          if (field!=NULL && colCnt<dataset_length)
          {
          double tmp;
          sscanf(field,"%lf",&tmp);
          dataset[colCnt]=tmp;
...
double euclidianDistanceLocal( double * local_data, int local_start, double * b, double dim ) {
  double sum = 0;
  for( int index=0; index<dim; index++ ) {
    sum += pow( ( local_data[local_start + index] - b[index] ), 2 );
  }
  return sqrt( sum );
}
